...
export default function Alert({ messages }) {
  ...
  // Setiap kali messages berubah (ada error baru), tampilkan lagi alertnya
  useEffect(() => {
  ...
  // Jangan render apapun kalau tidak ada pesan atau sudah ditutup
  ...
  return (
    <div className="bg-red-500 text-white p-3 rounded-xl text-sm relative">
      ...
      <button
        ...
        className="absolute right-3 top-2 text-white font-bold text-lg leading-none"
      >
        ×
      </button>

      {/* Tampilkan setiap pesan error di baris terpisah */}
      {messages.map((msg, i) => (
