...
const Home = ({}) => {
  ...
  const loadWallet = () => {
    ...
    .then((response) => {
      ...
      .catch((e) => alert(e)).finally(() =>     setLoadingWallet(false));
  };
  useEffect(() => {
    loadWallet();
    reload()
    ...
    <main className="px-5 py-8 lg:p-10 bg-slate-900 border border-slate-800 rounded-tl-3xl rounded-tr-3xl shadow flex flex-col gap-10 h-[calc(100vh_-_80px)] overflow-y-auto">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
        ...
        <div className="flex items-center gap-3">
          <a onClick={(e) => setShowTransaction(true)} className="btn">
            Add Transaction
          </a>
        </div>
      </div>
      ...
      <div className="-mb-3"> 
        ...
        {loadingWallet ? <>loading wallet....</>:wallets &&
      ...
      <div className="w-full max-w-[700px]">
        ...
        {!loading &&!hasMore&& transactions && transactions.length != 0 && <>nyampe batas</>}
        {!loading && transactions && transactions.length == 0 && <>Kosong</>}
