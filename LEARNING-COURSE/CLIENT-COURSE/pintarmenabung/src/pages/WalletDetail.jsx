...
export default function WalletDetail() {
  ...
  // Filter transaksi client-side berdasarkan wallet dan bulan
  const filteredTransactions = transactions.filter((t) => {
    const matchWallet = String(t.wallet_id) === String(walletId);
    const txDate = new Date(t.date);
    const matchMonth = txDate.getMonth() + 1 === selectedMonth;
    const matchYear = txDate.getFullYear() === selectedYear;
    return matchWallet && matchMonth && matchYear;
  });
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.altKey && e.key === "n") { e.preventDefault(); setShowAddTransaction(true); }
      if (e.altKey && e.key === "t") { e.preventDefault(); setShowTransfer(true); }
      if (e.key === "Escape") { setShowAddTransaction(false); setShowTransfer(false); }
    };
  ...
  const handleNameKeyDown = async (e) => {
    ...
    try {
      await updateWallet(walletId, { name: editName });
  ...
  const shouldShowDate = (index) => {
    ...
    return filteredTransactions[index].date !== filteredTransactions[index - 1].date;
  };
      ...
      <main className="px-5 py-8 lg:p-10 bg-slate-900 border border-slate-800 rounded-tl-3xl rounded-tr-3xl shadow flex flex-col gap-10 h-[calc(100vh_-_80px)] overflow-y-auto">
        ...
        <div className="flex items-center gap-3.5">
          ...
          {isEditingName ? (
            <input
              ...
              onBlur={() => { setIsEditingName(false); setEditName(wallet.name); }}
        ...
        <div className="w-full max-w-[700px] mx-auto">
          ...
          <div className="pb-8 flex flex-col lg:flex-row lg:items-end justify-between gap-5">
            <div>
              <h2 className="text-lg text-slate-400 font-medium mb-1">Total balance</h2>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowAddTransaction(true)} className="btn">
                Add Transaction
              </button>
            </div>
          </div>
